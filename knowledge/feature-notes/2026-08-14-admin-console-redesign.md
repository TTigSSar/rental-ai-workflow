...
- ~~A flaky spec in `public-profile-page.component.spec.ts` is unresolved~~ **Resolved 2026-08-15, see M-035.** The earlier "each spec file gets its own module registry even under `isolate: false`" conclusion was itself wrong for shared singletons: `MockStore.overrideSelector()` mutates the selector object `createSelector()` exported once from `auth.selectors.ts`, and three guard specs (`admin.guard.spec.ts`, `auth.guard.spec.ts`, `guest.guard.spec.ts`) never released their overrides — that leak, not a TestBed teardown race, was the mechanism. Fixed globally in `src/testing/reset-mock-store-selectors.ts`.
